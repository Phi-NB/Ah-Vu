  ...
  const convertArr = (arr1: any) => {
    return arr1.map((item: any) => {
      ...
      return {
        ...
        reported_time_display2:
          data[2] && data[2][item.key] ? convertValue(data[2][item.key]) : "--",
        reported_time_display1:
          data[1] && data[1][item.key] ? convertValue(data[1][item.key]) : "--",
        reported_time_display0:
          data[0] && data[0][item.key] ? convertValue(data[0][item.key]) : "--",
      };
    });
  };
