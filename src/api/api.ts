...
export async function getData(symbol: any, tokenString: any) {
  let res: LooseObject;
  try {
    res = await axios.get(getExampleDataUrl(symbol ? symbol : obj), {
      headers: {
        Authorization: `Bearer ${tokenString ? tokenString : tokenStr}`,
      },
    });
    return res;
  } catch (error) {
